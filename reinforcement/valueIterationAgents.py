...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for _ in range(self.iterations):
            ...
            for state in self.mdp.getStates():
                ...
                if action:
                    v_update[state] = self.getQValue(state, action)
            self.values = v_update
    ...
    def computeActionFromValues(self, state):
        ...
        for action in self.mdp.getPossibleActions(state):
            for pair in self.mdp.getTransitionStatesAndProbs(state, action):
                updates[action] += pair[1] * (self.mdp.getReward(state, action, pair[0]) + self.discount * self.getValue(pair[0]))
        return updates.argMax()
        ...
        for s in self.mdp.getStates():
            ...
            for pred in self.mdp.getStates():
                for action in self.mdp.getPossibleActions(pred):
                    for next_s, p in self.mdp.getTransitionStatesAndProbs(pred, action):
                        if (next_s == s) & (p > 0):
                            preds.add(pred)
